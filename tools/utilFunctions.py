...
INT32_FAC = (2 ** 31) - 1
INT64_FAC = (2 ** 63) - 1
norm_fact = {'int16': INT16_FAC, 'int32': INT32_FAC, 'int64': INT64_FAC,
...
def peakDetection(mX, t):
    ...
    next_minor = np.where(mX[1:-1] > mX[2:], mX[1:-1],
                          0)  # locations higher than the next one
    prev_minor = np.where(mX[1:-1] > mX[:-2], mX[1:-1],
                          0)  # locations higher than the previous one
...
def peakInterp(mX, pX, ploc):
    ...
    iploc = ploc + 0.5 * (lval - rval) / (
    lval - 2 * val + rval)  # center of parabola
    ipmag = val - 0.25 * (lval - rval) * (iploc - ploc)  # magnitude of peaks
    ipphase = np.interp(iploc, np.arange(0, pX.size),
                        pX)  # phase of peaks by linear interpolation
...
def genBhLobe(x):
    ...
    for m in range(0, 4):  # iterate over the four sincs to sum
        y += consts[m] / 2 * (sinc(f - df * m, N) + sinc(f + df * m,
                                                         N))  # sum of scaled sinc functions
...
def genSpecSines_p(ipfreq, ipmag, ipphase, N, fs):
    ...
    for i in range(0, ipfreq.size):  # generate all sine spectral lobes
        ...
        lb = np.arange(binremainder - 4,
                       binremainder + 5)  # main lobe (real value) bins to read
        lmag = genBhLobe(lb) * 10 ** (
        ipmag[i] / 20)  # lobe magnitudes of the complex exponential
        ...
        for m in range(0, 9):
            if b[m] < 0:  # peak lobe crosses DC bin
                ...
                m] == hN:  # peak lobe in the limits of the spectrum
                Y[b[m]] += lmag[m] * np.exp(1j * ipphase[i]) + lmag[m] * np.exp(
                    -1j * ipphase[i])
            else:  # peak lobe in positive freq. range
                Y[b[m]] += lmag[m] * np.exp(1j * ipphase[i])
        Y[hN + 1:] = Y[
                     hN - 1:0:-1].conjugate()  # fill the negative part of the spectrum
    return Y
...
def sinewaveSynth(freqs, amp, H, fs):
    ...
    for l in range(freqs.size):  # iterate over all frames
        if (lastfreq == 0) & (freqs[l] == 0):  # if 0 freq add zeros
            ...
            freqs[l] > 0):  # if starting freq ramp up the amplitude
            ...
            freqs[l] == 0):  # if ending freq ramp down the amplitude
        ...
        lastphase = np.remainder(phase[H - 1],
                                 2 * np.pi)  # save phase to be use for next frame
...
def cleaningTrack(track, minTrackLength=3):
    ...
    trackBegs = \
    np.nonzero((track[:nFrames - 1] <= 0)  # begining of track contours
               & (track[1:] > 0))[0] + 1
    if track[0] > 0:
...
def f0Twm(pfreq, pmag, ef0max, minf0, maxf0, f0t=0):
    ...
    if f0t > 0:  # if stable f0 in previous frame
        shortlist = np.argwhere(np.abs(f0cf - f0t) < f0t / 2.0)[:,
                    0]  # use only peaks close to it
        maxc = np.argmax(f0cm)
        maxcfd = f0cf[maxc] % f0t
        if maxcfd > f0t / 2:
            maxcfd = f0t - maxcfd
        if (maxc not in shortlist) and (maxcfd > (
    ...
    f0, f0error = UF_C.twm(pfreq, pmag,
                           f0cf)  # call the TWM function with peak candidates

    if (f0 > 0) and (
        f0error < ef0max):  # accept and return f0 if below max error allowed
        return f0
    else:
        return 0
...
def sineSubtraction(x, N, H, sfreq, smag, sphase, fs):
    ...
    xr = np.delete(xr, range(
        hN))  # delete half of first window which was added in stftAnal
    xr = np.delete(xr, range(xr.size - hN,
